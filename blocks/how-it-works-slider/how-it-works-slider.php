...
<section class="bg-gradient-to-b from-[#0E375E] to-[#082541]">
    <div class="block_content py-24 px-[60px]">
        <div class="[_&_h2]:text-white [_&_h2]:text-[36px] [_&_h2]:font-semibold [_&_h2]:leading-[60px] 
            [_&_h2]:tracking-tight [_&_p]:text-white [_&_p]:text-[20px]
        ...
        <div class="pt-[50px]">
            <div id="how-it-works-slider" class="slider-container relative">
                <?php foreach ($steps as $key => $step) : ?>
                    <div class="w-full h-[516px] slider-item rounded-[20px] <?php echo $key === 0 ? 'active' : ''; ?>"
                        ...
                        <div class="text-content absolute bottom-16 left-16 text-white max-w-[890px]">
                            <span class="text-white text-[16px] font-semibold leading-[24px]"></span>
                            Step <?php echo $key + 1; ?>
                            </span>
                            <div class="[_&_h2]:text-[36px] [_&_h2]:font-semibold [_&_h2]:tracking-tight 
                            [_&_p]:text-white [_&_p]:text-[18px]"><?php echo $step['text']; ?></div>
                        </div>
                    </div>
                <?php endforeach; ?>
            </div>
            <div class="slider-navigation flex justify-between gap-4 mt-16 w-full px-[20px]">
                <?php foreach ($steps as $key => $step) : ?>
    ...
    <div class="block_content ">
        <div class="flex">
            <?php foreach ($columns as $key => $col) : ?>
               <div class="w/1/3 py-[80px] px-[60px]" style="background: linear-gradient(91deg, rgba(6, 46, 45, 0.74) 0.69%, rgba(12, 94, 93, 0.74) 99.32%), url(<?php echo $col["image"] ?>) lightgray -17.161px 0px / 229.539% 100% no-repeat;backdrop-filter: blur(2px);">
               </div>
            <?php endforeach ?>
        </div>
    </div>
