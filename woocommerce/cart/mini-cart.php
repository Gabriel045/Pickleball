...
$current_url = $_POST['page'] ? $_POST['page'] : "";
...
<ul
    ...
    <?php
        ...
        foreach (WC()->cart->get_cart() as $cart_item_key => $cart_item) {
            ...
            if ($_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters('woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key)) {
                ...
                $sale_price         =  $current_url == "checkout" ? wc_price($product->get_price()) : wc_price($product->get_sale_price());
    ...
    <li
        ...
        <a href="<?php echo esc_url($product_permalink); ?>">
            <figure class="figure-product w-[100px]">
                <?php
        ...
        <div class="flex flex-col w-1/2">
            <h4 class="product-name text-gray-900 font-semibold leading-[20px] text-[20px] mb-3">
...
<div class="more-recommended bg-[#FAFBFC] px-[30px] lg:px-[40px] py-[40px] mt-[50px] rounded-[20px]">
    ...
    <p class="text-[#060843] text-center text-[16px] mt-3">This special price is only available right now while you're
        checking out</p>

    <div id="multiple-items" class="mt-[30px] flex-wrap lg:flex-nowrap gap-[20px] flex flex-col">
            ...
            <figure>
                <a href="<?php echo esc_url(get_permalink($item->ID)); ?>">
                    <?php
                            ...
                            ?>
                    <img class="rounded-xl aspect-[0.8] object-cover w-[120px]"
                        src="<?php echo esc_url($medium_image[0]); ?>" alt="">
                </a>
            </figure>
            <div class="mt-0 flex flex-col justify-center <?php echo $current_url == "checkout" ? 'w-1/2' : '' ?>">
                <div class="flex items-center gap-[10px]">
                    <span class="stars"></span>
                    <span class="text-[14px] text-[rgba(71,84,103,0.60)] font-medium leading-[24px]">5.0 (59
                        Reviews)</span>
                </div>
                <h3 class="text-rich-black font-semibold leading-[18px] text-[18px] ">
                    <a class="hover:underline" href="<?php echo esc_url(get_permalink($item->ID)); ?>">
                        <?php echo $product->get_name(); ?>
                    </a>
                </h3>
                <p class="text-gray-paragrah leading-[18px] text-[16px] my-2">
                    <?php echo $product->get_short_description(); ?> </p>
                <div class="flex items-center gap-[10px]">
                    <span
                        class="mr-4 text-[#8498AB] text-[16px] lg:text-[18px] line-through"><?php echo $regular_price ?></span>
                    <span
                        class="text-[#13A513] font-semibold leading-[32px] text-[22px]"><?php echo $sale_price ?></span>
                </div>
            </div>

            <?php if ($current_url == "checkout") : ?>
            <div class="woocommerce-variation-add-to-cart variations_button md:!m-auto">
                <button type="submit"
                    class="add_to_cart_checkout btn !w-full single_add_to_cart_button button !text-[14px]">+
                    Add to Cart</button>
